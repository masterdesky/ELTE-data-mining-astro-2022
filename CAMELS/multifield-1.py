...
class MultifieldDataset1():
    '''
    
    '''
    def __init__(self,
                 mode, fmaps, fmaps_norm, fparams, splits,
                 memmap=True, seed=None, verbose=False):
        ...
        for channel, (fim, fnorm) in enumerate(zip(fmaps, fmaps_norm)):
            ...
            for rot in [0,1,2,3]:
                ...
                data[count_maps:count_maps+size_maps,channel,:,:] = data_rot
                params[count_maps:count_maps+size_maps]           = params_maps
                count_maps += size_maps

                data[count_maps:count_maps+size_maps,channel,:,:] = \
                                                    np.flip(data_rot, axis=1)
                params[count_maps:count_maps+size_maps]           = params_maps
                count_maps += size_maps
            
            if verbose:
                print('Channel %d contains %d maps'%(channel,count_maps))
